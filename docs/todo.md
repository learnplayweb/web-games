# 수정
1. 디버그 모드
2. 
3. 


# 버튼 없는 모든 모달: 2.5초 후 자동 & 모달 바깥 클릭 닫힘
# 의사결정 필요한 모달: 자동 닫힘 없음. / 바깥 클릭 닫힘 기본 없는 것
 - 바깥 클릭 닫힘 예외
 파츠 모달
-----------

캐릭터 상점 개발  
[완료] 인벤토리 / 구입 / 랜덤구입 / 머리 적용 / 조합 / 해체 / 이름 설정 및 변경


# 색상 시스템 
[완료] 구입 / 랜덤구입 / 적용 / 색 섞기 / 다시 섞기




# 효과(연출) 구입

# 캐릭터 저장 / 도움말 페이지

# 캐릭터 연동
1. 캐릭터 데이터 연동
- SaveManager에 저장된 현재 꼬무리 정보를 메인 화면에서 읽기
- 현재 머리/상체/하체/색상/패턴/그라데이션을 그대로 표시
- 캐릭터 렌더링 코드를 공통화

2. 메인 화면 연동
- 상점에서 적용한 캐릭터가 메인 화면에 동일하게 나타나는지 확인
- 이름도 함께 표시
  * 을/를 자동 적용
----------------
  - 메인 캐릭터 이름 적용 (구매 후)
  function 을를(이름) {
    const 마지막글자 = 이름.trim().at(-1);
    const 코드 = 마지막글자?.charCodeAt(0) - 0xAC00;

    return 코드 >= 0 && 코드 <= 11171 && 코드 % 28 !== 0 ? '을' : '를';
  }
- 한 자리에 있지 않고 일정 시간마다 여기 저기 (몇 개 지점 지정, 하단에 나타날 땐 가까이 온 느낌으로 좀 크게?)

3. 게임 화면 연동
- games/clock에서도 동일한 저장 데이터를 읽어 캐릭터 표시
- 게임별 캐릭터 표시 영역에 맞춰 크기만 조정
- 캐릭터 데이터 자체는 게임마다 별도로 관리하지 않음
- 

4. 애니메이션 기반 구조 구축
- head, body, left-arm, right-arm, left-leg, right-leg 등을 독립적으로 애니메이션할 수 있게 함
- 먼저 아주 간단한 idle 애니메이션 하나만 구현

5. 애니메이션 추가
- 게임 중 행동 애니메이션
- 정답/보상 등 이벤트 애니메이션
- 메인화면에서 터치 시 반응

6. 연출(효과) 추가 (상점에서 구입)
- 파티클 터짐 등


# 애니메이션 적용 / 정오답 연출
게임화면 내 캐릭터 위치: 어울림. 자리차지x & 랜덤 위치 (다른 요소와 겹치지는 않으면서)

Commit Summary

Add random character placement system

---

다음 작업은 게임 화면과 메인 화면에 캐릭터를 표시하는 시스템만 구현한다.

수정 파일

* `index.html`
* `games/clock/index.html`
* `shared/header.js` (필요 시)
* `shared/character-display.js` (신규)
* `shared/character-display.css` (신규)

## 생성 파일

* `shared/character-display.js`
* `shared/character-display.css`

## 목표

현재 저장된 캐릭터를 메인 화면과 게임 화면에 자연스럽게 표시한다.

## 요구사항

### 1. 표시 대상

현재 저장된 캐릭터를 표시한다.

이름은 표시하지 않는다.

### 2. 표시 위치

캐릭터는 화면을 가리지 않는 후보 위치 중 하나를 랜덤으로 선택하여 표시한다.

예)

* 좌측 상단
* 우측 상단
* 좌측 하단
* 우측 하단

후보 위치는 UI 요소(시계, 입력 영역, 버튼, 헤더 등)와 겹치지 않도록 미리 정의한다.

게임 시작 또는 화면 진입 시마다 후보 위치 중 하나를 랜덤으로 선택한다.

### 3. 화면별 동작

메인 화면

* 자연스럽게 배치한다.

게임 화면

* 시계와 겹치지 않는 위치에 표시한다.
* 게임 플레이를 방해하지 않는다.

### 4. 애니메이션

기존 캐릭터 애니메이션을 그대로 사용한다.

* 둥실둥실
* 살랑살랑
* 자연스러운 흔들림

### 5. 공통화

캐릭터 표시 기능은 공통 컴포넌트로 구현한다.

향후 다른 화면에서도 동일한 방식으로 사용할 수 있도록 작성한다.

### 제외 사항

아직 구현하지 않는다.

* 클릭 기능
* 표정 변화
* 눈 깜박임
* 콤보 연출
* 정답/오답 연출

## 구현 원칙

* 후보 위치는 CSS 클래스로 관리한다.
* JavaScript는 후보 위치 중 하나만 랜덤 선택한다.
* 기존 UI를 가리지 않도록 배치한다.

변경된 파일만 출력한다.

구현 완료 후 요구사항별 완료/미완료 체크리스트를 함께 보고한다.

------------

# 꾸밈 세션